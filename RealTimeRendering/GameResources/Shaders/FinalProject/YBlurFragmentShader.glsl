...
float offset_x = 1.0 / textureSize;
float offset_y = 1.0 / textureSize;

float stepSize = 1 / textureSize;
...
void main() {
    ...
    vec2 coords = TexCoords - vec2(0.0, stepSize * 3.0);
    vec4 sum = vec4(0.0);
    for(int i = 0; i < 7; i++) {
        vec4 tap = texture2D(TextureSampler, coords);
		sum += weights[i] * tap;
		coords += vec2(0.0, stepSize * 1.0);
    }
    ...
    Color = vec4(texture(TextureSampler, TexCoords).xyz, 1.0);
    //Color = sum; //texture(TextureSampler, TexCoords);
    // float kernel_sum = 0.0;
    // for(int i = 0; i < 25; i++) {
    //     kernel_sum += kernel[i];
    // }
